  ...
  return (
    <main className="flex flex-col items-center justify-center flex-grow text-center px-4 py-8">
      ...
      <motion.p className="text-3xl md:text-4xl text-gray-700 font-semibold mb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
        Bienvenido, <strong className="text-blue-900">{username}</strong>
      </motion.p>

      <motion.section className="mb-12 w-full max-w-3xl bg-white/90 border border-white/20 rounded-xl shadow-lg px-6 py-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-3">
          <input
            ...
            className="px-4 py-2.5 rounded-full border border-slate-300 w-full sm:w-80 shadow-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all"
          />
          <button 
            onClick={handleSearch}
            className="bg-gradient-to-r from-teal-600 to-blue-700 hover:from-teal-700 hover:to-blue-800 text-white px-8 py-2.5 rounded-full transition duration-300 transform hover:scale-105 active:scale-95 shadow-md font-semibold"
          >
            Buscar
          </button>
        </div>
      ...
      <motion.section className="w-full max-w-screen-xl bg-white/90 border border-white/20 rounded-xl p-6 shadow-md" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
        <h2 className="text-xl font-bold text-left mb-4 text-slate-800">Categorías Temáticas</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {tematicas.map((tema, index) => (
            <Link 
              key={index} 
              href={`/bienvenida/colecciones?busqueda=${encodeURIComponent(tema.nombre)}`}
              className="bg-gradient-to-br from-[#001f3f] to-teal-600 text-white p-4 rounded-lg text-center shadow-md hover:shadow-xl hover:scale-105 active:scale-[0.98] transition transform duration-300 cursor-pointer flex flex-col items-center justify-center min-h-[120px]"
            >
              {tema.icono && (
