...
// --- INTERFACES DEFINITIVAS (basadas en tu JSON real) ---
...
// --- COMPONENTES (sin cambios, asegúrate que estén en tu archivo) ---
...
function LibroModal({ libro, onClose, todosLosLibros }: { libro: Libro; onClose: () => void; todosLosLibros: Libro[] }) {
    ...
    return (
        <AnimatePresence>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} onClick={(e) => e.stopPropagation()} className="bg-white rounded-xl shadow-2xl max-w-3xl w-full flex flex-col gap-6 p-6 relative overflow-y-auto max-h-[90vh]">
              <button onClick={onClose} className="absolute top-3 right-3 text-slate-400 hover:text-slate-800 transition rounded-full p-1" aria-label="Cerrar modal"><FaTimes size={20} /></button>
              <div className="flex flex-col md:flex-row gap-6">
                <div className="w-full md:w-1/3 flex-shrink-0"><Image src={libro.portada} alt={libro.nombre} width={200} height={300} className="object-cover rounded-lg shadow-lg mx-auto" /></div>
                <div className="flex flex-col">
                  ...
                  <div className="text-slate-600 text-justify space-y-3 mb-6">{libro.descripcion?.split('\n').map((p, i) => <p key={i}>{p}</p>) || 'No hay descripción disponible.'}</div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-auto pt-4 border-t">
                    <button onClick={handleDownloadTxt} className="flex items-center justify-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 font-semibold rounded-lg hover:bg-slate-200 transition-all active:scale-95"><FaDownload /> Descargar</button>
                    <Link href={`/leer/${libro.id}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 transition-all active:scale-95"><FaBookOpen /> Leer en línea</Link>
                  </div>
                </div>
              </div>
              {librosRelacionados.length > 0 && (
                <div className="mt-6 pt-6 border-t">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-4">
                    {librosRelacionados.map(relacionado => (<div key={relacionado.id} className="group cursor-pointer" title={relacionado.nombre}><div className="aspect-[2/3] relative rounded-md overflow-hidden shadow-sm transition-all group-hover:shadow-lg group-hover:-translate-y-1"><Image src={relacionado.portada} alt={relacionado.nombre} layout="fill" objectFit="cover" /></div></div>))}
                  </div>
                </div>
              )}
            </motion.div>
...
function LibroCard({ libro, onSelect }: { libro: Libro; onSelect: () => void }) {
    return (
        <motion.div className="flex-shrink-0 group cursor-pointer" variants={{ hidden: { y: 20, opacity: 0 }, visible: { y: 0, opacity: 1 } }} onClick={onSelect} layout>
            <motion.div className="w-full h-[210px] md:h-[240px] relative rounded-lg overflow-hidden shadow-md bg-slate-200 transition-all duration-300 hover:shadow-xl" whileHover={{ y: -8, scale: 1.05 }} transition={{ type: 'spring', stiffness: 300, damping: 15 }}>
...
function CategoriaEstante({ categoria, libros, onLibroSelect }: { categoria: string; libros: Libro[]; onLibroSelect: (libro: Libro) => void; }) {
    ...
    return (
        <motion.section key={categoria} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.5 }} layout>
            <div className="flex items-center justify-between mb-4"><h2 className="text-2xl md:text-3xl font-bold text-slate-800">{categoria}</h2><div className="flex items-center gap-2"><button onClick={() => scroll('left')} disabled={!puedeScrollIzquierda} className="p-2 rounded-full bg-slate-200 hover:bg-slate-300 transition disabled:opacity-30 disabled:cursor-not-allowed" aria-label="Scroll Izquierda"><FaChevronLeft className="text-slate-600" /></button><button onClick={() => scroll('right')} disabled={!puedeScrollDerecha} className="p-2 rounded-full bg-slate-200 hover:bg-slate-300 transition disabled:opacity-30 disabled:cursor-not-allowed" aria-label="Scroll Derecha"><FaChevronRight className="text-slate-600" /></button></div></div>
            <div className="relative"><motion.div ref={scrollRef} className="flex gap-4 md:gap-6 overflow-x-auto pb-4 -mx-4 px-4 scroll-smooth snap-x snap-mandatory" style={{ scrollbarWidth: 'none' }} layout>{libros.map((libro) => (<div key={libro.id} className="snap-center md:snap-start flex-shrink-0 w-[150px] md:w-[160px]"><LibroCard libro={libro} onSelect={() => onLibroSelect(libro)} /></div>))}</motion.div></div>
        </motion.section>
    );
}
function SkeletonCard() {
    return (<div className="w-[150px] md:w-[160px] flex-shrink-0"><div className="w-full h-[225px] md:h-[240px] bg-slate-200 rounded-lg animate-pulse"></div><div className="h-4 bg-slate-200 rounded mt-3 w-3/4 mx-auto animate-pulse"></div></div>);
}
      ...
      try {
        ...
        if (json.data && Array.isArray(json.data)) {
          const librosProcesados: Libro[] = json.data.map((libro: LibroFromAPI) => {
            ...
            const urlBase = 'https://ihcbackend.onrender.com';
            const portadaUrl = libro.portada?.formats?.small?.url || libro.portada?.url || '/uploads/default.jpg';
          
            return {
              id: libro.id,
              nombre: libro.nombre,
              portada: `${urlBase}${portadaUrl}`,
